...
void ReferbishAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    juce::dsp::ProcessSpec spec{
        ...
        .maximumBlockSize = (uint32_t)samplesPerBlock,
        .numChannels = (uint32_t)std::max(getTotalNumInputChannels(), getTotalNumOutputChannels()),
    };

    channelStep = std::vector<float>(spec.numChannels, {0.f});

    inputDiffusers = std::vector<schroeder_allpass<float>>(inputDiffuserCount);
    ...
    for (int i = 0; i < inputDiffuserCount; ++i)
    {
        ...
        inputDiffusers[i].m_max_diffusion_delay_ms = 50.f;
        inputDiffusers[i].m_diffusion_delay_samples =
            utilities::milliseconds_to_samples((float)(i + 1) / (float)inputDiffuserCount * 40.f, sampleRate);
...
void ReferbishAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    if (!bypass)
    {
        ...
        for (int sample = 0; sample < numSamples; ++sample)
        {
            ...
            {
                diffuser.process_step(channelStep);
            }
...
juce::AudioProcessorValueTreeState::ParameterLayout ReferbishAudioProcessor::CreateParameterLayout()
{
    ...
    // layout.add(std::make_unique<juce::AudioParameterFloat>(SHIMMERAMOUNT_PARAM, SHIMMERAMOUNT_PARAM, 0.1f,
    // .65f, 0.2f));
    ...
    // layout.add(std::make_unique<juce::AudioParameterFloat>(DRIVEGAIN_PARAM,
    // DRIVEGAIN_PARAM, 1.f, 30.f, 2.f));
